...
import { addDays, today, todayStr, nowStr, parseNowStr, getNextDate, freqWeight, isCompletedLateByDate, getStatus, isCompletedStatus, isLateStatus, parseJSON, hashPwd, getFileIcon, sanitizeFileName } from "./helpers";
...
export default function App() {
  ...
  const refreshScoringData=async()=>{
    const[{data:pjd},{data:scd}]=await Promise.all([supabase.from("projects").select("id,name,lead_eid,steps,quality_rating,quality_rated_at,quality_on_time"),supabase.from("support_cases").select("id,eid,difficulty,created,content,category").eq("deleted",false)]);
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      for(const tpl of due){
        // Hàng ngày luôn khóa hạn trong ngày (không dùng deadline_days), các tần suất khác vẫn tự do chọn số ngày
        const deadline=tpl.frequency==="daily"?todayStr:addDays(today,tpl.deadline_days||7);
        const newTask={id:`t${Date.now()}_${Math.random().toString(36).slice(2,6)}`,title:`🔄 ${tpl.title}`,description:tpl.description||"",dept:tpl.dept,eid:tpl.eid,prio:tpl.prio,deadline,completed:false,created:todayStr,progress:0,attachments:"[]",collab_eids:tpl.collab_eids||"[]",collab_note:tpl.collab_note||"",template_id:tpl.id,weight:freqWeight(tpl.frequency),history:JSON.stringify([{action:`Tạo tự động từ mẫu định kỳ "${tpl.title}"`,by:"Hệ thống",at:nowStr()}]),rating:"",rating_note:"",rated_by:"",rated_at:"",late_reason:"",late_note:""};
  ...
  const navItems=[{id:"dashboard",icon:"📊",label:"Tổng quan"},{id:"myqueue",icon:"🗂️",label:"Việc chờ xử lý",shortLabel:"Chờ xử lý",badge:myQueueTotal},{id:"work",icon:"💼",label:"Công việc"},{id:"calendar",icon:"🗓️",label:"Lịch (Deadline/Trực)",shortLabel:"Lịch"},{id:"documents",icon:"📁",label:"Văn bản"},{id:"reports",icon:"📈",label:"Báo cáo"},{id:"employees",icon:"👥",label:"Nhân viên"},{id:"feedback",icon:"💡",label:"Góp ý"},{id:"help",icon:"📘",label:"Hướng dẫn"},...(canSeeAll?[{id:"activity",icon:"📜",label:"Nhật ký"}]:[]),...(currentUser?.role==="admin"?[{id:"security",icon:"🔐",label:"Bảo mật"}]:[])];
  ...
  return(<>
    <div className={darkMode?"qlcv-dark":""} style={{display:"flex",flexDirection:isMobile?"column":"row",height:"100dvh",fontFamily:"system-ui,sans-serif",background:darkMode?"#0f172a":"#f8fafc",overflow:"hidden",zoom:zoom}}>
      ...
      {extRequestModal&&(
        <div onClick={()=>setExtRequestModal(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:120,padding:16}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:14,width:"100%",maxWidth:440,boxShadow:"0 12px 40px rgba(0,0,0,0.2)"}}>
            ...
            <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:14}}>
              <div style={{padding:"10px 14px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:8,fontSize:12,color:"#1d4ed8"}}>Hạn chót hiện tại: <b>{extRequestModal.deadline}</b>. Đề xuất sẽ được gửi tới người giao việc để duyệt.</div>
              <div>
      ...
      {extDecideModal&&(
        <div onClick={()=>setExtDecideModal(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:120,padding:16}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:14,width:"100%",maxWidth:440,boxShadow:"0 12px 40px rgba(0,0,0,0.2)"}}>
            ...
            <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:14}}>
              <div style={{padding:"10px 14px",background:"#f8fafc",borderRadius:8,fontSize:12,color:"#374151"}}>
                <div style={{marginBottom:4}}><b>{extDecideModal.task.ext_requested_by}</b> đề xuất gia hạn đến <b>{extDecideModal.task.ext_proposed}</b> (hạn hiện tại: {extDecideModal.task.deadline})</div>
                ...
                <div style={{background:"#f8fafc",borderRadius:10,border:"1px solid #e5e7eb",padding:16,marginBottom:16}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                      ...
                      <div><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:3}}>Deadline (ngày sau khi tạo)</label>{templateForm.data.frequency==="daily"?<div style={{...inp,background:"#f9fafb",color:"#6b7280"}} title="Nhiệm vụ hàng ngày luôn có hạn trong ngày">Trong ngày 🔒</div>:<input type="number" min={1} max={90} value={templateForm.data.deadline_days} onChange={e=>setTemplateForm(f=>({...f,data:{...f.data,deadline_days:Number(e.target.value)}}))} style={inp}/>}</div>
      ...
      <TaskModal
        ...
        openApproveModal={openApproveModal} rejectCompletionRequest={rejectCompletionRequest} remindApproval={remindApproval}
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6,maxHeight:220,overflowY:"auto"}}>
                  {bulkFromOpenTasks.map(t=>(
                    <label key={t.id} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"8px 10px",border:"1px solid "+(bulkSelectedIds.includes(t.id)?"#a5b4fc":"#e5e7eb"),borderRadius:8,background:bulkSelectedIds.includes(t.id)?"#eef2ff":"#fff",cursor:"pointer"}}>
                      ...
                      <div style={{minWidth:0}}><div style={{fontSize:13,wordBreak:"break-word"}}>{t.title}</div><div style={{fontSize:11,color:"#9ca3af",marginTop:2}}>Hạn: {t.deadline} · {t.dept}</div></div>
                    </label>
                  ))}
                </div>
                <div>
                  ...
                  <select value={bulkToEid} onChange={e=>setBulkToEid(e.target.value)} style={inp}>
                    <option value="">— Chọn nhân viên —</option>
                    {(employees||[]).filter(e=>e.dept===getEmp(bulkFromEid)?.dept&&e.id!==bulkFromEid).map(e=><option key={e.id} value={e.id}>{e.name} ({e.role})</option>)}
                  </select>
                </div>
              </>)
            )}

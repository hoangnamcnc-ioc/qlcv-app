    ...
    <div className={darkMode?"qlcv-dark":""} style={{display:"flex",flexDirection:isMobile?"column":"row",height:"100dvh",fontFamily:"system-ui,sans-serif",background:darkMode?"#0f172a":"#f8fafc",overflow:"hidden",zoom:zoom}}>
        ...
        <ErrorBoundary key={view} label={getViewMeta(view)?.label||"màn hình này"}>
          ...
          {view==="dashboard"&&(
            <Dashboard
              ...
              atRiskTasks={atRiskTasks} weeklyDigest={weeklyDigest} watchList={watchList} dataHealth={dataHealth} execNarrative={execNarrative}
      ...
      {showQuickStart&&currentUser&&(()=>{
        ...
        return(<div onClick={dismiss} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:120,padding:16}}><div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:14,width:"100%",maxWidth:460,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 12px 40px rgba(0,0,0,0.25)"}}>
          <div style={{background:"linear-gradient(135deg,#4f46e5,#6366f1)",color:"#fff",padding:"18px 20px"}}><div style={{fontSize:18,fontWeight:700}}>👋 Chào {currentUser.full_name}!</div><div style={{fontSize:12.5,opacity:0.9,marginTop:3}}>Vài việc nên làm với vai trò <b>{roleLabel}</b>:</div></div>
          <div style={{padding:"14px 20px"}}>{list.map((t,i)=>(<div key={i} style={{display:"flex",gap:10,padding:"8px 0",borderBottom:i<list.length-1?"1px solid #f3f4f6":"none",fontSize:13.5,color:"#374151",lineHeight:1.5}}><span style={{color:"#6366f1",fontWeight:700}}>{i+1}.</span><span>{t}</span></div>))}</div>
          <div style={{padding:"12px 20px 18px",display:"flex",gap:8,justifyContent:"flex-end",alignItems:"center",flexWrap:"wrap"}}><button onClick={()=>{dismiss();setView("help");}} style={{background:"none",border:"none",color:"#6366f1",cursor:"pointer",fontSize:13,textDecoration:"underline"}}>Xem hướng dẫn đầy đủ</button><button onClick={dismiss} style={{background:"#4f46e5",color:"#fff",border:"none",borderRadius:8,padding:"9px 20px",cursor:"pointer",fontSize:13.5,fontWeight:600}}>Bắt đầu</button></div>
        </div></div>);
      })()}

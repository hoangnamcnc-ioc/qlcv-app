...
const emptyTemplate=(emps,dept)=>{const d=dept||DEPTS[0];const f=emps.find(e=>e.dept===d&&!e.no_kpi);return{title:"",description:"",dept:d,eid:f?.id||"",prio:"medium",frequency:"monthly",deadline_days:7,active:true,next_date:todayStr,collab_eids:"[]",collab_note:""};};
...
export default function App() {
  ...
  const emptyTaskData=()=>{const dept=canSelfCreate?userDept:availableDepts[0];const first=(employees||[]).find(e=>e.dept===dept&&!e.no_kpi);return{title:"",description:"",dept,eid:canSelfCreate?currentUser.employee_id:(first?.id||""),prio:"medium",deadline:addDays(today,7),attachments:"[]",progress:0,collab_eids:"[]",collab_note:"",depends_on:""};};
  ...
  const changeTaskDept=v=>{const f=(employees||[]).find(e=>e.dept===v&&!e.no_kpi);setTaskForm(tf=>({...tf,data:{...tf.data,dept:v,eid:f?f.id:""}}));};
  ...
  const submitTask=async()=>{let{data,editId}=taskForm;if(!data.title||!data.deadline)return;
    ...
    if(canCreate&&(!data.eid||getEmp(data.eid)?.no_kpi||getEmp(data.eid)?.dept!==data.dept)){showToast("Vui lòng chọn lại người nhận việc hợp lệ (đúng phòng, không bị khóa nhận việc).","error");return;}

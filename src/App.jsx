...
import { DEPTS, DEPT_COLOR, deptLabel, isRankable, isAssignable, DEFAULT_DEPARTMENTS, setDepartments, ROLES_EMP, VI_MONTHS, VI_DAYS, ROLE_LABELS, ROLE_COLORS, FULL_ACCESS, CAN_CREATE, RATING, LATE_REASONS, OVERLOAD_DEFAULT, FREQUENCIES, STATUS, PRIO, STATUS_ORDER, LATE_COMPLETION_PENALTY } from "./constants";
...
const emptyTemplate=(emps,dept)=>{const d=dept||DEPTS[0];const f=emps.find(e=>e.dept===d&&isAssignable(e));return{title:"",description:"",dept:d,eid:f?.id||"",prio:"medium",frequency:"monthly",deadline_days:7,active:true,next_date:todayStr,collab_eids:"[]",collab_note:""};};
...
export default function App() {
  ...
  const submitTemplate=async()=>{const{editId}=templateForm;const data={...templateForm.data,deadline_days:templateForm.data.frequency==="daily"?0:templateForm.data.deadline_days};if(!data.title||!data.eid){showToast("Nhập tiêu đề và chọn người giao","error");return;}setSaving(true);if(editId){const old=recurringTemplates.find(t=>t.id===editId);const{error}=await supabase.from("recurring_templates").update(data).eq("id",editId);if(error){showToast("Lỗi lưu mẫu: "+(error.message||""),"error");setSaving(false);return;}setRecurringTemplates(p=>p.map(t=>t.id===editId?{...t,...data}:t));
    ...
    // đã sinh trước đó, để không còn hiển thị người cũ. Việc đã hoàn thành giữ nguyên (lịch sử điểm).
    if(old&&(old.eid!==data.eid||old.dept!==data.dept)){const patch={eid:data.eid,dept:data.dept};const{error:e2,data:upd}=await supabase.from("tasks").update(patch).eq("template_id",editId).eq("completed",false).select("id");if(!e2){const ids=new Set((upd||[]).map(r=>r.id));if(ids.size){setTasks(p=>p.map(t=>ids.has(t.id)?{...t,...patch}:t));showToast(`Đã cập nhật ${ids.size} việc định kỳ đang mở sang người/phòng mới`);}}}
  ...
  const emptyTaskData=()=>{const dept=canSelfCreate?userDept:availableDepts[0];const first=(employees||[]).find(e=>e.dept===dept&&isAssignable(e));return{title:"",description:"",dept,eid:canSelfCreate?currentUser.employee_id:(first?.id||""),prio:"medium",deadline:addDays(today,7),attachments:"[]",progress:0,collab_eids:"[]",collab_note:"",depends_on:""};};
  ...
  const suggestAssignee=dept=>{const cands=(employees||[]).filter(e=>e.dept===dept&&isAssignable(e)).map(e=>({e,load:activeLoadByEid[e.id]?.w||0,onTime:empReliability[e.id]?.onTimeRate??null}));if(!cands.length)return null;cands.sort((a,b)=>(a.load-b.load)||((b.onTime??70)-(a.onTime??70)));return cands[0];};
  ...
  const changeTaskDept=v=>{const f=(employees||[]).find(e=>e.dept===v&&isAssignable(e));setTaskForm(tf=>({...tf,data:{...tf.data,dept:v,eid:f?f.id:""}}));};
  ...
  const submitTask=async()=>{let{data,editId}=taskForm;if(!data.title||!data.deadline)return;
    ...
    // Chặn giao việc cho người không hợp lệ (chưa chọn, hoặc người bị khóa 🚫 KPI = không nhận việc) — tránh lưu nhầm.
    if(canCreate&&(!data.eid||!isAssignable(getEmp(data.eid))||getEmp(data.eid)?.dept!==data.dept)){showToast("Vui lòng chọn lại người nhận việc hợp lệ (đúng phòng, không phải BGĐ/khóa nhận việc).","error");return;}
  ...
  const navItems=[{id:"dashboard",icon:"📊",label:"Tổng quan"},{id:"myqueue",icon:"🗂️",label:"Việc chờ xử lý",shortLabel:"Chờ xử lý",badge:myQueueTotal},{id:"work",icon:"💼",label:"Công việc"},{id:"calendar",icon:"🗓️",label:"Lịch (Deadline/Trực)",shortLabel:"Lịch"},...(canSeeDocumentsTab?[{id:"documents",icon:"📁",label:"Văn bản",badge:docsNeedingAttentionCount}]:[]),{id:"chat",icon:"💬",label:"Chat"},{id:"reports",icon:"📈",label:"Báo cáo"},...(canSeeAll?[{id:"kpitrends",icon:"📉",label:"KPI & Xu hướng",shortLabel:"KPI"}]:[]),{id:"employees",icon:"👥",label:"QL Nhân sự",shortLabel:"Nhân sự"},{id:"feedback",icon:"💡",label:"Góp ý"},{id:"help",icon:"📘",label:"Hướng dẫn"},...(canSeeAll?[{id:"activity",icon:"📜",label:"Nhật ký"}]:[]),...(["admin","director"].includes(currentUser?.role)?[{id:"chatlearn",icon:"🧠",label:"Trợ lý học"}]:[]),...(currentUser?.role==="admin"?[{id:"security",icon:"🔐",label:"Bảo mật"}]:[])];
        ...
        <ErrorBoundary key={view} label={getViewMeta(view)?.label||"màn hình này"}>
          ...
          {view==="myqueue"&&(
            <MyQueue
              ...
              getEmp={getEmp} deptLeaderName={deptLeaderName}
          ...
          {view==="reports"&&(
            <Reports
              ...
              canExec={true} myEid={currentUser?.employee_id} rejectedCreateTasks={rejectedCreateTasks} computed={computedGlobal} monthlyScores={monthlyScores} snapshotMonth={snapshotMonth} syncManagerSnapshots={syncManagerSnapshots} currentUser={currentUser} overloadThreshold={overloadThreshold} kpiOnTime={kpiOnTime}
            />
          )}
          ...
          {view==="employees"&&(
            <Personnel
              ...
              deptOversight={deptOversight} setDeptOverseer={setDeptOverseer}
                ...
                <div style={{background:"#f8fafc",borderRadius:10,border:"1px solid #e5e7eb",padding:16,marginBottom:16}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                      <div><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:3}}>Phòng ban</label>{canSeeAll?<select value={templateForm.data.dept} onChange={e=>{const f=(employees||[]).find(emp=>emp.dept===e.target.value);setTemplateForm(tf=>({...tf,data:{...tf.data,dept:e.target.value,eid:f?.id||""}}));}} style={inp}>{DEPTS.map(d=><option key={d} value={d}>{deptLabel(d)}</option>)}</select>:<div style={{...inp,background:"#f9fafb"}}>{templateForm.data.dept}</div>}</div>
                      <div><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:3}}>Giao cho *</label><select value={templateForm.data.eid} onChange={e=>setTemplateForm(f=>({...f,data:{...f.data,eid:e.target.value}}))} style={inp}>{(employees||[]).filter(e=>e.dept===templateForm.data.dept&&isAssignable(e)).map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select></div>
          ...
          <div style={{padding:18,display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div>
              ...
              <select value={bulkFromEid} onChange={e=>{setBulkFromEid(e.target.value);setBulkSelectedIds([]);}} style={inp}>
                ...
                {(canAssignAllDepts?(employees||[]).filter(isAssignable):(employees||[]).filter(e=>e.dept===userDept&&isAssignable(e))).map(e=><option key={e.id} value={e.id}>{e.name} ({e.dept})</option>)}
              </select>
            </div>
            {bulkFromEid&&(
              bulkFromOpenTasks.length===0?(
                ...
                <div>
                  ...
                  <select value={bulkToEid} onChange={e=>setBulkToEid(e.target.value)} style={inp}>
                    <option value="">— Chọn nhân viên —</option>
                    {(employees||[]).filter(e=>e.dept===getEmp(bulkFromEid)?.dept&&e.id!==bulkFromEid&&isAssignable(e)).map(e=><option key={e.id} value={e.id}>{e.name} ({e.role})</option>)}
                  </select>
                </div>
              </>)
            )}

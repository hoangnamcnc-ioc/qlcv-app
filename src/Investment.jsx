  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {loading?(<div style={{padding:40,textAlign:"center",color:"#9ca3af"}}>Đang tải dự án…</div>):projects.length===0?(
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(2,1fr)",gap:12}}>
        {projects.map(p=>{const steps=parseJSON(p.steps,[]);const doneSteps=steps.filter(s=>s.status==="done").length;const pct=steps.length?Math.round(doneSteps/steps.length*100):0;const remain=(Number(p.total_budget)||0)-(Number(p.spent)||0);const overdueN=countOverdueSteps(steps);return(
          <div key={p.id} onClick={()=>setProjDetail(p)} style={{background:"#fff",borderRadius:12,border:"1px solid #e5e7eb",padding:16,cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 4px 16px rgba(0,0,0,0.08)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:10}}><div style={{fontWeight:600,fontSize:14,flex:1}}>{p.name}{overdueN>0&&<span style={{marginLeft:6,fontSize:10,background:"#fee2e2",color:"#b91c1c",padding:"1px 7px",borderRadius:8,fontWeight:600,whiteSpace:"nowrap"}}>⚠ {overdueN} trễ</span>}</div><span style={{background:DEPT_COLOR[p.dept]+"22",color:DEPT_COLOR[p.dept],fontSize:11,padding:"2px 8px",borderRadius:8,flexShrink:0}}>{p.dept}</span></div>

...
export default function AssistantChat({ employees, computed, calcMonthPerf, managerPerf, empReliability, activeLoadByEid, getEmp, isCompletedStatus, onOpenTask, onOpenProfile, onOpenHelp, isMobile, me, onCreate }) {
  ...
  const answer = (raw) => {
    ...
    // 1a) LIỆT KÊ việc theo TRẠNG THÁI (chờ duyệt / quá hạn / hoàn thành / đang làm) — có thể kèm người/phòng/kỳ
    ...
    const wantsList = has(qn, "danh sach", "liet ke", "nhung viec", "cac viec", "viec nao", "nhung nhiem vu", "cho xem", "xem cac", "liet", "co viec nao", "gom nhung");
    if (has(qn, "cho duyet", "cho phe duyet", "phe duyet", "dang cho duyet", "cho duyet hoan thanh", "cho xet duyet")) return statusList("Việc đang chờ duyệt", t => t.status === "pending_approval");
    if (wantsList && has(qn, "qua han", "tre han", "tre hen", "cham tre")) return statusList("Việc quá hạn", t => t.status === "overdue");
    if (wantsList && has(qn, "hoan thanh", "da xong", "hoan tat", "da lam xong")) return statusList("Việc đã hoàn thành", t => isCompletedStatus(t.status));
    if (wantsList && has(qn, "dang lam", "dang thuc hien", "dang mo", "chua xong", "chua hoan thanh", "dang xu ly")) return statusList("Việc đang thực hiện", t => !isCompletedStatus(t.status) && t.status !== "pending_approval");

    // 1b) PHÒNG nào nhiều việc TRỄ/QUÁ HẠN nhất (đếm theo phòng — khác với "tỷ lệ hoàn thành theo phòng")
    if (has(qn, "phong") && has(qn, "qua han", "tre", "cham") && has(qn, "nhieu", "nhat", "phong nao")) { const byDept = {}; for (const t of computed) { if (!t.dept || !inPeriod(t)) continue; if (t.status === "overdue" || t.status === "completed_late") byDept[t.dept] = (byDept[t.dept] || 0) + 1; } const rows = Object.entries(byDept).sort((a, b) => b[1] - a[1]); if (!rows.length) return { text: `Không phòng nào có việc trễ/quá hạn${period ? ` (${period.label})` : ""}. 👍` }; return { text: `Phòng nhiều việc trễ/quá hạn nhất${period ? ` (${period.label})` : ""}:`, bars: rows.map(([d, v]) => ({ label: d, value: v })) }; }

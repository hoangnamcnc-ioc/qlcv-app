...
import { DEPTS, STATUS, deptLabel, isRankable } from "../constants";
...
export default function AssistantChat({ employees, computed, calcMonthPerf, managerPerf, empReliability, activeLoadByEid, getEmp, isCompletedStatus, onOpenTask, onOpenProfile, onOpenHelp, isMobile, me, onCreate }) {
  ...
  const clarify = (base, chip) => { const p = strip(chip); const dept = (DEPTS || []).find(d => p.includes(strip(deptLabel(d))) || p.includes(strip(d))); const suffix = dept ? ` phòng ${dept}` : p.includes("thang nay") ? " tháng này" : ""; send((base + suffix).trim()); };

      ...
      <div style={{ background: "#fff", borderRadius: 10, border: "1px solid #e5e7eb", overflow: "hidden" }}>
        ...
        {isMobile ? (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, tableLayout: "fixed" }}>
            <thead><tr style={{ background: "#f9fafb" }}>{[["Họ và tên","24%"],["Chức vụ","15%"],["Đang TH","12%"],["Sắp HH","9%"],["Quá hạn","9%"],["HT quá hạn","10%"],["Tỷ lệ","11%"],["","10%"]].map(([h,w]) => <th key={h} style={{ padding: "8px 14px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#6b7280", borderBottom: "1px solid #e5e7eb", width: w }}>{h}</th>)}</tr></thead>
            <tbody>
              {deptEmps(empDeptTab).length === 0 && <tr><td colSpan={8} style={{ padding: 24, textAlign: "center", color: "#9ca3af" }}>Chưa có nhân viên.</td></tr>}
              {deptEmps(empDeptTab).map(emp => {
                const et = computed.filter(t => t.eid === emp.id);
                const ov = et.filter(t => t.status === "overdue").length;
                const cl = et.filter(t => t.status === "completed_late").length;

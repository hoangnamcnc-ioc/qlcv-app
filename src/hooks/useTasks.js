...
export default function useTasks({ tasks, setTasks, employees, currentUser, canSeeAll, userDept, canCreate, showToast, getEmp, setModal, setSaving, delegations }) {
  ...
  // Phạm vi hiện trong DANH SÁCH Nhiệm vụ (không ảnh hưởng Báo cáo — báo cáo dùng computedGlobal, đã tải đủ):
  //  • admin/BGĐ (canSeeAll): thấy mọi việc.
  //  • Quản lý phòng (canCreate): thấy mọi việc PHÒNG mình.
  //  • NHÂN VIÊN thường: CHỈ thấy việc của CHÍNH MÌNH + việc PHỐI HỢP (không thấy việc người khác).
  const canSeeTask = useMemo(() => (t) => {
    if (!currentUser) return false;
    if (canSeeAll) return true;
    const myEid = currentUser.employee_id;
    if (t.eid === myEid) return true;
    if (parseJSON(t.collab_eids, []).includes(myEid)) return true;
    if (canCreate && userDept && t.dept === userDept) return true; // trưởng/phó phòng: cả phòng
    return false;
  }, [currentUser, canSeeAll, canCreate, userDept]);
  ...
  const stats = useMemo(() => { const s = computed.reduce((a, t) => { a.total += 1; a[t.status] = (a[t.status] || 0) + 1; return a; }, { total: 0, on_time: 0, nearly_due: 0, overdue: 0, pending_approval: 0, completed_late: 0, completed: 0, pending_create: 0 }); s.pending_create = pendingCreateTasks.length; return s; }, [computed, pendingCreateTasks]);
  ...
  const statsW = useMemo(() => {
    const r = computed.reduce((a, t) => { const wt = t.weight ?? 1; a.total += wt; a[t.status] = (a[t.status] || 0) + wt; return a; }, { total: 0, on_time: 0, nearly_due: 0, overdue: 0, pending_approval: 0, completed_late: 0, completed: 0, pending_create: 0 });
    r.pending_create = pendingCreateTasks.reduce((s, t) => s + (t.weight ?? 1), 0);
